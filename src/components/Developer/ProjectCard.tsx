...
export default function ProjectCard ({name, description, user, forms, status, id, openForm, showForms}:cardProps) {
    return (
        <div className="project-card p-5 bg-black border border-zinc-700 rounded-lg shadow-lg grid grid-cols-1 gap-3">
            ...
            <div className="latest-endpoint border-t border-zinc-700 py-3">
                <div className="title-and-copy-endpoint-icon flex items-center justify-between py-3">
                    <h4 className="text-sm font-semibold text-zinc-200">Latest Endpoint</h4>
                    <button className="flex items-center justify-center h-10 w-10 cursor-pointer rounded-full hover:bg-zinc-900/50 transition">
                        <Icon name="Copy" size={16} className="text-zinc-500 hover:text-zinc-300 transition" onClick={() => {
                            navigator.clipboard.writeText(`https://api.formo.com/project/${id}/endpoint`);
                            alert('Endpoint copied to clipboard!');
                        }} />
                    </button>
                </div>
                <div className="flex items-center gap-2 bg-zinc-900/70 p-3 rounded-lg">
                    {/* <Icon name="Terminal" size={16} className="text-zinc-500" strokeWidth={2} /> */}
                    <span className="text-xs font-mono text-zinc-400">https://api.formo.com/project/{id}/endpoint</span>
                </div>
            </div>
        </div>
    )
