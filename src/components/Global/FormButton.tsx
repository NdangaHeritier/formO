...
const FormButton = ({variant="primary",type="button",onClick, children, disabled = false}:Props) => {
    const variation ={
    ...
    return(
        <button
            ...
            onClick={onClick}
            disabled={disabled}
            className={`${variation[variant]} ${disabled ? `opacity-40`: ``} font-semibold duration-300 px-4 py-2 rounded-md text-sm flex items-center justify-center gap-2 cursor-pointer`}
        >
            {children}                    
        </button>
    )
